...
<%-- <script src="${pageContext.request.contextPath }/resources/assets/plugins/jquery.js"></script>
<script src="/webjars/jquery-ui/1.12.1/jquery-ui.min.js"></script> --%>
<script>
	
	//ajax
	function goMonthAgo(param){
		...
		httpRequest.onreadystatechange = function(data){
			if(httpRequest.readyState == 4){
				if(httpRequest.status == 200){
					$('#detailList').empty();
					$('#detailList').html(httpRequest.responseText);
				}
			}
		}
		...
		httpRequest.open('post', '../resources/ajax/depositDetailMonthAgo.jsp', true);
		httpRequest.setRequestHeader('content-type', 'application/x-www-form-urlencoded');
		httpRequest.send('accountNumber=' + param);
	}
...
<div class="container">
  ...
  <div class="section-title text-center">
            ...
            <p>총 잔액 : ${ String.format( "%,d",depositAccount.getBalance() ) }<br>
...
<div class="container">
  ...
  <div class="btn-group">
    <button type="button" class="btn btn-style-one">이번 달 내역</button>
    <button type="button" class="btn btn-style-one" id="goMonthAgo" onclick="goMonthAgo(${depositAccount.accountNumber})">이전 달 내역</button>
  ...
  <table class="table table-hover">
    ...
    <tbody id="detailList">
     ...
     <c:forEach items="${ depositDetailList }" var="depositDetail" varStatus="loop" >
      ...
      <tr <c:if test="${ loop.count mod 2 eq 1 }">class="even"</c:if> >
       <td >
